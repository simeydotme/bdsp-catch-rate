...
const ballRate = (pkmn, ball, hp, lvl, status = 1) => {
  const mod = {
    difficulty: difficulty(lvl.us, lvl.them),
    low_level: level_check(lvl.them),
    status: status,
    grass: 1,
    ball: ball_rate(pkmn, ball, lvl),
  };
...
const healthRate = (pkmn, ball, lvl, status = 1) => {
  return Array.from({ length: 100 }).map((_, hp) => {
    const rate = ballRate(pkmn, ball, hp + 1, lvl, status);
